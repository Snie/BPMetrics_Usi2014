module.exports = function(grunt) {
  ...
  function genTarget(target) {
    ...
    var newGens = [
      ...
      generateFromTemplate('templates/runner.html', {target: target}, 'database/runner-' + target + '.html')];
    return newGens;
  }
  ...
  grunt.task.registerMultiTask('test', 'Run <target> tests under Karma', function() {
    var done = this.async();
    var karmaConfig = require('karma/lib/config').parseConfig(require('path').resolve('database/karma-config.js'), {});
    var config = targetConfig[this.target];
    karmaConfig.files = ['database/runner.js'].concat(config.src, config.test);
  ...
  grunt.task.registerMultiTask('sauce', 'Run <target> tests under Karma on Saucelabs', function() {
    var done = this.async();
    var karmaConfig = require('karma/lib/config').parseConfig(require('path').resolve('database/karma-config-ci.js'), {});
    var config = targetConfig[this.target];
    karmaConfig.files = ['database/runner.js'].concat(config.src, config.test);
  ...
  grunt.task.registerTask('clean', 'Remove files generated by grunt', function() {
    grunt.file.expand('web-animations*').concat(grunt.file.expand('database/runner-*.html')).concat(grunt.file.expand('inter-*')).forEach(function(file) {
